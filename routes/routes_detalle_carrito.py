...
@router.get('/{id}',response_model=DetalleCarritoResponder)
def obtener_detalles_carrito_por_id(id:str,db:Session=Depends(get_db),current_user: Usuario = Depends(get_current_user)):
    return get_detalle_carrito_por_id(db,id,current_user.id)
...
@router.patch('/{id}',response_model=DetalleCarritoResponder)
def actualizar_detalles_carrito(id:str,detalle_carrito:DetalleCarritoActualizar,db:Session=Depends(get_db),current_user: Usuario = Depends(get_current_user)):
    return patch_detalle_carrito(db,id,detalle_carrito,current_user.id)

@router.delete('/{id}',response_model=DetalleCarritoResponder)
def borrar_detalles_carrito(id:str,db:Session=Depends(get_db),current_user: Usuario = Depends(get_current_user)):
    return delete_detalle_carrito(db,id,current_user.id)